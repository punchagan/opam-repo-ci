...
- [system-b.0.0.1] is using a restricted prefix in its name
  ...
  * a-1 (tag: initial-state, master)
  $ opam-repo-ci-local --repo="." --branch=new-branch-1 --lint-only --no-web-server
  Error "Command "opam-ci-check" "lint" "-r" "." "-n" "b.0.0.1,b.0.0.2,b.0.0.3,system-b.0.0.1" exited with status 1"
...
of a package [a_1] that conflicts with the existing [a-1] package
  ...
  * a-1 (tag: initial-state, master)
  $ opam-repo-ci-local --repo="." --branch=new-branch-1 --lint-only --no-web-server
  Error "Command "opam-ci-check" "lint" "-r" "." "-n" "a_1.0.0.1" exited with status 1"
...
test various positive and negative cases
  ...
  * a-1 (tag: initial-state)
  ...
  Error "Command "opam-ci-check" "lint" "-r" "." "-c" "field1.0.0.2" "-n" "fieffind.0.0.1,fieffinder.0.0.1,fielf.0.0.1" exited with status 1"
